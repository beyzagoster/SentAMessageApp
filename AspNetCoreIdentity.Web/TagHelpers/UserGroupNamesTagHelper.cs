...
namespace AspNetCoreIdentity.Web.TagHelpers
{
    public class UserGroupNamesTagHelper: TagHelper
    {
        ...
        public override async Task ProcessAsync(TagHelperContext context, TagHelperOutput output)
        {
            ...
            var userRoles = await _context.UserAndGroupMatching.Where(p => p.UserId == user.Id).ToListAsync();

            var stringBuilder = new StringBuilder();

            userRoles.ToList().ForEach(x =>
            {
                var groupToUpdate = _context.SiteUserGroup.FirstOrDefault(p => p.Id == x.UserGroupId);
